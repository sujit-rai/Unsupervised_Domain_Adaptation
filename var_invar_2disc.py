...
def dom_classifier(x,isTrainable=True,reuse=False):
    with tf.variable_scope("dom_classifier") as scope:
        if reuse:
            scope.reuse_variables();
...
v_disc = dom_classifier(v_latent_code)
...
dom_cls_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='dom_classifier')


update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS);
with tf.control_dependencies(update_ops):
    enc_dec_op = tf.train.AdamOptimizer(learning_rate = enc_dec_lr).minimize(recon_loss, var_list = enc_dec_params);
    disc_op = tf.train.AdamOptimizer(learning_rate = 0.0001).minimize(disc_loss, var_list=disc_params)
    inv_enc_op = tf.train.AdamOptimizer(learning_rate = 0.0001).minimize(inv_enc_loss, var_list=inv_enc_params)
    v_enc_op = tf.train.AdamOptimizer(learning_rate = 0.0001).minimize(v_enc_loss, var_list = v_enc_params+dom_cls_params)
        ...
        for epoch in range(epochs):
            ...
            for batch in range(n_batches):
                ...
                for i in range(k):
                    ...
                    _, _v_enc_loss, _merged_all = sess.run([v_enc_op, v_enc_loss, merged_all], feed_dict = fd)
                if(iterations%20==0):
