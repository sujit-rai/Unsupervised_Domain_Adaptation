...
def inv_dom(x,isTrainable=True,reuse=False):
    with tf.variable_scope("inv_dom") as scope:
        if reuse:
            scope.reuse_variables();
...
def inv_dom_classifier(x,isTrainable=True,reuse=False):
    with tf.variable_scope("inv_dom_classifier") as scope:
        ...
        disc_logits = tf.layers.dense(x, units = 1, 
...
def inv_cls_classifier(x,isTrainable=True,reuse=False):
    with tf.variable_scope("inv_cls_classifier") as scope:
        ...
        cls_logits = tf.layers.dense(x, units = 10, 
            kernel_initializer= initializer, activation = None,trainable=isTrainable,reuse=reuse,name='fc_layer')
...
def v_dom(x,isTrainable=True,reuse=False):
    with tf.variable_scope("v_dom") as scope:
        if reuse:
            scope.reuse_variables();
...
inv_dom_logits = inv_dom_classifier(inv_dom(inv_latent_code))
inv_cls_logits = inv_cls_classifier(inv_dom(inv_latent_code, isTrainable=True, reuse = True))


v_latent_code = v_encoder(X)
v_dom_logits = v_dom_classifier(v_dom(v_latent_code))
v_cls_logits = v_cls_classifier(v_dom(v_latent_code, isTrainable = True, reuse=True))
...
inv_dom_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="inv_dom")
v_dom_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="v_dom")
...
with tf.control_dependencies(update_ops):
    ...
    inv_cls_op = tf.train.AdamOptimizer(learning_rate = inv_cls_lr).minimize(inv_cls_loss,var_list=inv_enc_params + inv_cls_params + inv_dom_params)
    v_e_cls_op = tf.train.AdamOptimizer(learning_rate = v_e_cls_lr).minimize(v_e_cls_loss,var_list=v_enc_params)
    v_d_cls_op = tf.train.AdamOptimizer(learning_rate = v_d_cls_lr).minimize(v_d_cls_loss,var_list=v_cls_params + v_dom_params)
    
    inv_e_d_op = tf.train.AdamOptimizer(learning_rate = inv_e_d_lr).minimize(inv_e_disc_loss,var_list=inv_enc_params)
    v_d_op = tf.train.AdamOptimizer(learning_rate = v_e_d_lr).minimize(v_disc_loss,var_list=v_enc_params + v_disc_params + v_dom_params)
    inv_d_d_op = tf.train.AdamOptimizer(learning_rate = inv_d_d_lr).minimize(inv_d_disc_loss,var_list=inv_disc_params + inv_dom_params)
...
log_dir = "var_invar_2dcac_2dec"

init = tf.group(tf.global_variables_initializer(), tf.local_variables_initializer())

checkpoint_path = "./var_invar_2dcac_2dec"
            ...
            for batch in range(n_batches):
                ...
                for k in range(1):
                    for j in range(2):
                        ...
                        _,_inv_cls_loss,_inv_acc = sess.run([inv_cls_op, inv_cls_loss, inv_acc], feed_dict=produce_batch("source"))
                    for l in range(1):
                        _,_inv_d_disc_loss, _inv_dom_logits = sess.run([inv_d_d_op, inv_d_disc_loss, inv_dom_logits], feed_dict=produce_batch("both"))
                ...
                if(batch%100 == 0):
                    ...
                    print("inv logits mean : ", np.mean(sigmoid(_inv_dom_logits)))
                    print("inv_logits std : ", np.std(sigmoid(_inv_dom_logits)))
